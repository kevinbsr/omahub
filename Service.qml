...
Item {
  ...
  readonly property alias integrations: integrationHost

  HubIntegrations { id: integrationHost; hubService: root }

  // The hub's open tab, shared by every monitor's copy of the panel. There is
  // one bar -- so one Panel.qml -- per monitor, and they used to agree on the
  // tab by writing it to shell.json and waiting for the write to come back.
  // That round trip is not cheap: persisting re-syncs every plugin widget on
  // every bar, and the file watcher then re-applies the same config and does it
  // again -- measured with qmlprofiler at ~100 ms plus ~100 ms of blocked QML
  // thread per tab change, on all monitors at once. This service is a single
  // instance, so a property here reaches every panel without touching disk.
  property string currentTab: ""

  // ONE weather source for every monitor. It used to live in Panel.qml, and
  // there is one Panel per monitor, so three sources ran side by side: measured
  // at shell start-up, 6 wttr.in and 3 open-meteo requests for a single
  ...
  WeatherSource {
    id: weatherSource
    hub: root.settingsHub
  }

  // ---- The ONE hub panel.
  //
  // Each monitor's bar used to load its own Panel.qml -- the whole thing, every
  // tab it had shown, and until recently its own weather fetcher -- although
  // only one panel is ever on screen. Now there is a single panel here, and a
  // bar widget "attaches" it when it opens it: the panel takes that widget's
  // bar, anchor and screen (see BarWidget.attachPanel). That also removes the
  // need for copies to agree on the tab at all.
  //
  // `panelWanted` latches: it is set by the first widget to arrive, so the
  // panel is built once at start-up rather than on somebody's first click, and
  // it survives its current host being destroyed.
  property var panelHost: null
  ...
  // IPC lives on the singleton service rather than on BarWidget.qml. There is
  // one widget per monitor, and registering the same target from each copy
  // made whichever monitor loaded first win. The current panel host already
  // knows which bar/screen owns the shared panel, so it is the stable route.
  function hubHost() {
    return root.panelHost || root.settingsHub
  }

  function refreshHub() {
    var host = root.hubHost()
    if (host && typeof host.refresh === "function") host.refresh()
  }

  function openHubTab(name, toggleOnly) {
    var host = root.hubHost()
    if (!host) return
    if (toggleOnly && typeof host.toggleTab === "function") host.toggleTab(name)
    else if (typeof host.openTab === "function") host.openTab(name)
  }
  ...
  IpcHandler {
    ...
    function cycleFormat(): void {
      var host = root.hubHost()
      if (host && typeof host.cycleFormat === "function") host.cycleFormat()
    }
    function toggleWeekStart(): void {
      var host = root.hubHost()
      if (host && typeof host.toggleWeekStart === "function") host.toggleWeekStart()
    }
    function open(): void {
      var host = root.hubHost()
    ...
    function toggle(): void {
      var host = root.hubHost()
      if (host && typeof host.togglePanel === "function") host.togglePanel()
    }
    function tab(name: string): void { root.openHubTab(name, false) }
    function tabToggle(name: string): void { root.openHubTab(name, true) }
    function refreshWeather(): void {
      if (root.weather && typeof root.weather.refresh === "function") root.weather.refresh()
    }
  }
